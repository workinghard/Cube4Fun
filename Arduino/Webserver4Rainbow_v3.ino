...
#define MY_CONTROL_PASS "Ss12345678" // need to be the same and 10 char length
#define MY_WRITE_PASS   "Ww12345678" // need to be the same and 10 char length
...
void processRequest(EthernetClient client) {
  ...
  if ( debugFunc == false ) {
    if (readBufferCompare2(MY_CONTROL_PASS, 10) > -1) { // Streammode, expect frames
      DEBUG_PRINTLN_TXT("Set mode to 1");
      setServerMode(1);
    }else if(readBufferCompare2(MY_WRITE_PASS, 10) > -1 ) { // Stream write mode
